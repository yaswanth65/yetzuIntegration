...
export default function AdminCertificatesPage() {
  ...
  const handleVerify = async (certificateId: string) => {
    ...
    try {
      const response = await CertificateAPI.admin.verifyCertificate(certificateId);
      if (response?.success) {
    ...
    <div className="font-sans flex flex-col">
      ...
      <div className="bg-white px-6 md:px-10 py-4 border-b border-gray-200 shrink-0">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-medium text-[#021165] sm:text-3xl md:text-4xl">Manage Certificates</h1>
              <p className="text-sm text-gray-500 mt-1">View and verify all certificates</p>
            </div>
              ...
              <table className="w-full text-left border-collapse min-w-[900px]">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="py-4 px-6 text-[14px] font-bold text-gray-900">Educator</th>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">{cert.source} {cert.sessionTitle ? `• ${cert.sessionTitle}` : ""}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6 text-sm text-gray-600">{cert.studentName || cert.studentId || "N/A"}</td>
                      <td className="py-4 px-6 text-sm text-gray-600">{cert.educatorName || cert.educatorId || "N/A"}</td>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 text-xs bg-green-100 text-green-700 rounded-lg hover:bg-green-200 disabled:opacity-50"
                          >
                            {verifyingId === cert.certificateId ? (
