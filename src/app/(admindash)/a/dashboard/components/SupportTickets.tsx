...
import { Eye, Clock, MessageSquare, CheckCircle2, XCircle } from "lucide-react";
...
const getStatusStyles = (status: Status) => {
  switch (status) {
    ...
    case "Submitted":
      return { color: "text-blue-600", bg: "bg-blue-50", icon: MessageSquare };
    case "Completed":
      return { color: "text-emerald-600", bg: "bg-emerald-50", icon: CheckCircle2 };
    case "Rejected":
      return { color: "text-gray-500", bg: "bg-gray-50", icon: XCircle };
  }
};
...
export default function SupportTickets({ tickets }: SupportTicketsProps) {
  ...
  const supportItems: SupportItem[] = (tickets || []).map((item: any, index: number) => ({
    id: item.ticketCode || item.id || `TK-${index + 1}`,
    description: item.description || item.subject || "Support ticket",
    priority:
      String(item.priority || "Low").toLowerCase() === "high"
        ? "High"
  ...
  return (
    <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden group hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-500">
      <div className="flex items-center justify-between p-6 sm:p-8">
        ...
        <Link
          href="/a/support"
          className="px-4 py-2 text-xs font-bold text-[#042BFD] bg-blue-50 hover:bg-blue-100 rounded-xl transition-all uppercase tracking-widest"
        >
          View All
        </Link>
      </div>
              ...
              supportItems.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group/row">
                    <td className="px-8 py-5 text-sm font-bold text-gray-900" title={item.id}>
                      #{shortenId(item.id)}
                    </td>
                    ...
                    <td className="px-8 py-5">
                      <span
                        className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${getPriorityStyles(
                          item.priority
                        )}`}
                      >
                        {item.priority}
            ...
            return (
              <div key={item.id} className="p-6 active:bg-gray-50 transition-colors">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-black text-[#021165] bg-blue-50 px-2 py-1 rounded-lg" title={item.id}>
                    #{shortenId(item.id)}
                  </span>
