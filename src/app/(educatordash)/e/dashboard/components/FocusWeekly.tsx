    ...
    <div className="mt-8 mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {focusData.map((item) => (
          <div
            key={item.id}
            className="bg-white rounded-2xl border border-gray-100 flex flex-col h-[240px] shadow-[0_2px_10px_rgba(0,0,0,0.02)] relative overflow-hidden group"
          >
            {/* Background Gradient */}
            <div
              className={`absolute top-0 left-0 right-0 h-40 bg-gradient-to-b ${item.gradientFrom} to-white opacity-60 pointer-events-none`}
            ></div>

            {/* Watermark Icon */}
            <div
              className={`absolute top-1 left-3 ${item.watermarkColor} pointer-events-none`}
            >
              <Image
                src={item.watermarkIcon}
                alt="icon"
                width={80}
                height={80}
              />
            ...
            <div className="p-5 flex flex-col justify-between h-full relative z-10">
              <div>
                <div className="flex justify-end items-start mb-6 h-6">
                  {item.badgeLabel && (
                    <span
                      className={`text-[9px] font-bold px-2.5 py-1 uppercase rounded-full ${item.badgeColor}`}
                    >
                      {item.badgeLabel}
                    </span>
                  )}
