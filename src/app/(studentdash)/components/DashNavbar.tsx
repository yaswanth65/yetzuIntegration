...
export default function DashNavbar({
  ...
  const getNotifTitle = (notif: any) => {
    if (notif.type === "new_message") return notif.from?.name || "New Message";
    return notif.title || "Notification";
  };

  const getNotifDescription = (notif: any) => {
    if (notif.type === "assignment_feedback") return notif.educatorRemark || "Feedback received";
    if (notif.type === "new_message") return notif.content || "";
    return notif.subtitle || notif.message || "";
  };
    ...
    switch (notif.type) {
      case "assignment_feedback":
        if (notif.assignmentId) {
          router.push(`/s/assignments/${notif.assignmentId}`);
        }
        break;
      case "new_message":
        router.push("/s/chat");
