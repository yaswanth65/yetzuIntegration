    ...
    return (
        <div className="p-1 bg-[#F9FAFB] min-h-screen font-['Inter']">
            ...
            <div className="flex gap-4 mb-8">
                <button className="px-6 py-2 bg-[#042BFD] text-white rounded-lg text-sm font-medium shadow-sm transition-colors">
                    Certificates
                </button>
            ...
            <div className="flex flex-col lg:flex-row gap-6 mb-10 h-auto lg:h-[300px]">
                ...
                <div className="flex-[2] bg-gradient-to-r from-[#042BFD] to-[#2F54FF] rounded-[20px] p-8 md:p-10 text-white relative overflow-hidden flex flex-col justify-center">
                    <div className="relative z-10 max-w-lg">
                        ...
                        <div className="flex items-center gap-6">
                            <Link href="/s/webinars" className="bg-white text-[#042BFD] px-6 py-3 rounded-lg font-semibold text-sm hover:bg-blue-50 transition-colors">
                                Explore more Webinars
                            </Link>
                            <Link href="/s/courses" className="flex items-center gap-2 font-medium text-sm hover:text-blue-100 transition-colors">
                ...
                <div className="flex-1 bg-[#EBF0FF] rounded-[20px] p-6 flex flex-col items-center justify-center text-center min-w-[280px]">
                    ...
                    <span className="text-[#021165] text-5xl font-bold">{webinars.length}</span>
                </div>
            </div>

            {/* Completed Webinars Grid */}
            <div>
                <h2 className="text-2xl font-bold text-gray-800 mb-6">Completed Webinars</h2>

                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {[1, 2, 3, 4].map((i) => (
                            <div key={i} className="h-[300px] bg-gray-200 rounded-[20px] animate-pulse"></div>
                        ))}
                    </div>
                ) : webinars.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {webinars.map((webinar) => (
                            <div key={webinar.id} className="bg-white p-4 rounded-[20px] border border-gray-100 shadow-sm flex flex-col hover:shadow-md transition-shadow">
                                ...
                                <h3 className="font-semibold text-gray-900 mb-2 truncate text-lg" title={webinar.title}>{webinar.title}</h3>
                                <p className="text-gray-500 text-xs mb-4 line-clamp-2 h-8">{webinar.description}</p>
