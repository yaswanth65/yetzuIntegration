...
export default function AssignmentsTable({ data = [] }: AssignmentsTableProps) {
    ...
    if (!data || data.length === 0) {
        return <div className="p-6 text-center text-gray-500">No assignments found.</div>;
    }
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden relative min-h-[400px]">
                            ...
                            {currentData.map((assignment) => (
                                <tr key={assignment._id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4 text-sm text-gray-600">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-400 line-clamp-1">{assignment.courseId}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-gray-600">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${getStatusStyle(assignment.status)}`}>
                ...
                <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50">
                    <p className="text-sm text-gray-500">
                        Showing <span className="font-medium">{Math.min((currentPage - 1) * pageSize + 1, data.length)}</span> to <span className="font-medium">{Math.min(currentPage * pageSize, data.length)}</span> of <span className="font-medium">{data.length}</span> results
                    </p>
                        ...
                        <div className="p-6 space-y-4">
                            <div className="flex items-start gap-4 p-3 rounded-xl bg-gray-50 border border-gray-100">
                                ...
                                <div>
                                    ...
                                    <a href={`mailto:${selectedEducator.email}`} className="text-sm font-medium text-gray-900 hover:text-blue-600 transition-colors truncate block">
                            ...
                            <div className="flex items-start gap-4 p-3 rounded-xl bg-gray-50 border border-gray-100">
                                ...
                                <div>
                                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-0.5">Status</p>
